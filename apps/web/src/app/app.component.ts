import { Component, inject } from "@angular/core";
import { RouterLink, RouterLinkActive, RouterOutlet } from "@angular/router";
import { ApiService } from "./core/api.service";
import { AuthPageComponent } from "./features/auth/auth-page.component";

@Component({
  selector: "dd-root",
  standalone: true,
  imports: [AuthPageComponent, RouterLink, RouterLinkActive, RouterOutlet],
  template: `
    @if (!api.authReady()) {
      <main class="auth-shell">
        <section class="panel empty-state">
          <strong>Loading workspace</strong>
          <span>Checking your session.</span>
        </section>
      </main>
    } @else if (!api.session()) {
      <dd-auth-page />
    } @else {
      <div class="shell">
        <aside class="sidebar">
          <a routerLink="/overview" class="brand">
            <span class="brand-mark" aria-hidden="true">U</span>
            <span>
              <strong>UIpen</strong>
              <small>{{ api.session()?.workspace?.name }}</small>
            </span>
          </a>
          <nav class="nav" aria-label="Primary">
            <a routerLink="/overview" routerLinkActive="active">Overview <span>Start</span></a>
            <a routerLink="/audit" routerLinkActive="active">Audit <span>Debt</span></a>
            <a routerLink="/tokens" routerLinkActive="active">Tokens <span>Forge</span></a>
            <a routerLink="/scans" routerLinkActive="active">Scans <span>History</span></a>
            <a routerLink="/settings" routerLinkActive="active">Settings <span>Team</span></a>
          </nav>
          <button class="button quiet signout-button" type="button" (click)="api.logout()">Sign out</button>
        </aside>

        <main class="content">
          <router-outlet />
        </main>
      </div>
    }
  `,
})
export class AppComponent {
  readonly api = inject(ApiService);

  constructor() {
    void this.api.initializeAuth();
  }
}
